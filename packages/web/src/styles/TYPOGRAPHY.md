...
Applied to: the app sidebar, `/leads`, `/leads/:id`, the intake forms, — as
of the CRM design-language pass — `/crm/tickets`, `/crm/tickets/archived`,
`/clients/:id` and `/policies/:id` — and the Agency Command Center at
`/dashboard/management-alt` (PAC-138).

Still excluded: the **management** and **service** prototype dashboards. Both
are slated for replacement and still carry the patterns this document exists to
stop (`text-[10px]`, hand-rolled pills and buttons, raw hex, and the four
undefined `--emerald` / `--red` / `--amber` / `--font-mono` variables that
render as no colour at all). Don't copy from them.
